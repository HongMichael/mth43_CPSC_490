...
model_medium_training: models/model_medium_training.ckpt.meta
...
models/model_medium_training.ckpt.meta: data/${FILE} ${SOURCES_FOR_TRAINING}
	mkdir -p models/
	python3 -m tsmlstarterbot.train --model_name model_medium_training --data data/${FILE} --games_limit 500 --steps 5000 --seed ${SEED}

models/model_short_training.ckpt.meta: data/${FILE} ${SOURCES_FOR_TRAINING}
	mkdir -p models/
	python3 -m tsmlstarterbot.train --model_name model_short_training --data data/${FILE} --games_limit 10 --steps 50 --seed ${SEED}
...
compare: bin/compare.sh MyBot.py models/model_medium_training bin/halite
	bin/compare.sh MyBot.py MyBot.py
